...
For daily cron on headless servers, add to `crontab -e`:
```
0 6 * * * xvfb-run claude --dangerously-skip-permissions -p '/job-auto-apply'
```
...
```
claude-job-auto-apply/
├── config.json                            # User settings (daily target, search, etc.)
├── .mcp.json                              # Playwright MCP config (auto-generated)
├── CLAUDE.md                              # Auto-generated by bootstrap
│
├── .claude/skills/job-auto-apply/
│   ├── SKILL.md                           # Core pipeline logic
│   ├── SETUP.md                           # Bootstrap setup guide
